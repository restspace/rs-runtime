...
const sendTokenUrl = async (
  msg: Message,
  context: ServiceContext<IAdapter>,
  serviceConfig: AccountServiceConfig,
  subservice: AccountSubservice,
): Promise<Message> => {
  if (msg.url.servicePathElements.length < 1) {
    return msg.setStatus(400, "Missing email");
  }

  const user = await getUserFromEmail(
    context,
    serviceConfig.userUrlPattern,
    msg,
  ...
  const expiry = new Date().getTime() +
    (subservice.tokenExpiryMins || 30) * 1000 * 60;
  user.tokenExpiry = new Date(expiry);

  const saveOutMsg = await saveUser(
    context,
    serviceConfig.userUrlPattern,
    msg,
    user,
  ...
  (user as AuthUser & { returnPageUrl: string })["returnPageUrl"] =
    returnPageUrl.toString();
  ...
  emailMsg.url = Url.fromPathPattern(
    serviceConfig.emailSendUrlPattern,
    msg.url,
    { email: user.email },
  );

...
import {
  ...
  isSameDomainBrowserRequest,
  isTrustedLoginDomain,
} from "../auth/browserOrigin.ts";
...
function buildJwtPayload(
  ...
  if (Array.isArray(jwtUserProps)) {
    for (const prop of jwtUserProps) {
      ...
      if (blockedJwtUserProps.has(prop) || reservedJwtClaims.has(prop)) {
        continue;
      }
...
function isHttpsRequest(msg: Message): boolean {
  const forwardedProto = (msg.getHeader("x-forwarded-proto") || "")
    .toLowerCase();
...
async function setJwt(
  ...
  const jwt = await runtimeConfig.authoriser.getJwtForPayload(
    payload,
    timeToExpirySecs,
  );
...
function getMfaCookieName(serviceConfig: AuthServiceConfig): string {
  const configured =
    (serviceConfig.mfa?.mfaCookieName || "rs-mfa").toString().trim() ||
    "rs-mfa";
  // Never allow collisions with the real auth cookie.
  if (configured === "rs-auth") {
    runtimeConfig.logger.warn(
      "auth.mfaCookieName must not be rs-auth; using rs-mfa instead",
    );
    return "rs-mfa";
  }
  return configured;
...
function redactUserForClient(user: any) {
  ...
  delete out.authLockout;
  delete out.secretEnc;
  return out;
}

function loginLockoutParams(
  config: AuthServiceConfig,
): { maxAttempts: number; lockMinutes: number } {
  const maxAttempts = typeof config.loginLockout?.maxAttempts === "number" &&
      config.loginLockout.maxAttempts > 0
    ? Math.floor(config.loginLockout.maxAttempts)
    : 10;
  const lockMinutes = typeof config.loginLockout?.lockMinutes === "number" &&
      config.loginLockout.lockMinutes > 0
    ? config.loginLockout.lockMinutes
    : 10;
  return { maxAttempts, lockMinutes };
}

function getLoginLockout(fullUser: Record<string, any>): StoredLoginLockout {
  const lockout = fullUser.authLockout;
  return lockout && typeof lockout === "object" ? lockout : {};
}

function isLoginLocked(lockout: StoredLoginLockout): boolean {
  if (!lockout.lockUntil) return false;
  return new Date(lockout.lockUntil).getTime() > Date.now();
}
...
async function recordFailedLogin(
  ...
  config: AuthServiceConfig,
  fullUser: Record<string, any>,
): Promise<{ locked: boolean; saveMsg?: Message }> {
  const { maxAttempts, lockMinutes } = loginLockoutParams(config);
  const storedLockout = getLoginLockout(fullUser);
  const lockout = storedLockout.lockUntil
    ? { failedAttempts: 0 }
    : { ...storedLockout };
  ...
  if (lockout.failedAttempts >= maxAttempts) {
    ...
    lockout.lockUntil = lockUntil.toISOString();
  }
  fullUser.authLockout = lockout;
  const saveMsg = await saveUser(
    context,
    config.userUrlPattern,
    msg,
    fullUser as AuthUser,
    true,
  );
  return { locked: isLoginLocked(lockout), saveMsg };
}
...
async function clearLoginLockout(
  ...
  const lockout = getLoginLockout(fullUser);
  if (!lockout.failedAttempts && !lockout.lockUntil && !fullUser.authLockout) {
    return undefined;
  }
  delete fullUser.authLockout;
...
function userRequiresTotp(fullUser: any): boolean {
  ...
  if (
    fullUser.totp && typeof fullUser.totp === "object" &&
    fullUser.totp.enabled === true
  ) return true;
  return false;
}

async function totpVerifyInternal(
  msg: Message,
  context: SimpleServiceContext,
  ...
  const verifyUrl = totpBase.endsWith("/")
    ? `${totpBase}verify`
    : `${totpBase}/verify`;
  ...
  if (!verifyOut.ok) {
    return {
      ok: false,
      required: true,
      error: (await verifyOut.data?.asString().catch(() => "")) ||
        "verify error",
    };
  }
...
async function login(
  ...
  const email = (userSpec?.email || "").toString().trim().toLowerCase();
  const password = (userSpec?.password || "").toString();
  if (!email || !password) {
    return msg.setStatus(400, "Missing email or password");
  }
  const fullUser = await getUserFromEmail(
    context,
    userUrlPattern,
    msg,
    email,
  ...
  if (!fullUser) {
    return msg.setStatus(401, "Invalid credentials");
  }
  const lockout = getLoginLockout(fullUser as Record<string, any>);
  if (isLoginLocked(lockout)) {
    return msg.setStatus(423, "Locked");
  }
  const user = new AuthUser(fullUser);
  const match = await user.matchPassword(password);
  if (!match) {
    const failed = await recordFailedLogin(
      msg,
      context,
      config,
      fullUser as Record<string, any>,
    );
    if (failed.saveMsg && !failed.saveMsg.ok) return failed.saveMsg;
    return msg.setStatus(
      failed.locked ? 423 : 401,
      failed.locked ? "Locked" : "Invalid credentials",
    );
  }
  const clearMsg = await clearLoginLockout(
    msg,
    context,
    config,
    fullUser as Record<string, any>,
  );
  if (clearMsg && !clearMsg.ok) {
    return clearMsg;
  }
  ...
  try {
    ...
    if (mfaMode === "challenge" && userRequiresTotp(fullUser)) {
      ...
      const mfaJwtResult = await setMfaJwt(
        msg,
    ...
    const payload = buildJwtPayload(
      user,
      fullUser as unknown as Record<string, unknown>,
      config.jwtUserProps,
    );
    const jwtResult = await setJwt(
      msg,
      payload,
      config.sessionTimeoutMins || 30,
      {
        setCookie: loginCookiePolicy.setCookie,
        strictCookie: loginCookiePolicy.strictCookie,
      },
    ...
    if (newMsg.ok) {
      if (redirUrl.query["redirect"]?.length) {
        ...
        try {
          ...
          isValidRedirect = !redirectUrl.domain ||
            (redirectUrl.scheme === msg.url.scheme &&
              redirectUrl.domain === msg.url.domain);
        } catch {}
        if (isValidRedirect) {
          context.logger.info(
            `authenticationService redirect from ${msg.url} by query arg to ${redirectTarget}`,
          );
          newMsg.redirect(new Url(redirectTarget), true);
        } else {
          context.logger.warn(
            `authenticationService blocked redirect to external URL: ${redirectTarget}`,
          );
          // Redirect to login page without redirect param instead
          const { redirect: _redirect, ...queryWithoutRedirect } =
            redirUrl.query;
        ...
        context.logger.info(
          `authenticationService redirect from ${msg.url} to add succeed query to ${redirUrl}`,
        );
        newMsg.redirect(redirUrl, true);
      }
    } else {
      ...
      context.logger.info(
        `authenticationService redirect from ${msg.url} to add fail query to ${redirUrl}`,
      );
      newMsg.redirect(redirUrl, true);
    }
  }
...
service.postPath("mfa/totp", async (msg, context, config) => {
  ...
  const authResult = await runtimeConfig.authoriser.verifyJwtHeader(
    authHeader,
    mfaCookie,
    msg.url.path,
  );
  if (
    typeof authResult === "string" || !authResult.email ||
    (authResult as any).mfaPending !== true
  ) {
    return msg.setStatus(401, "Invalid mfa token");
  }
  const body = msg.data
    ? await msg.data.asJson().catch(() => ({} as any))
    : ({} as any);
  ...
  const verify = await totpVerifyInternal(
    msg,
    context,
    config,
    authResult.email,
...
service.setUser(async (msg, _context, serviceConfig) => {
  ...
  const attemptedAuthSource: AuthSource | undefined = authHeader
    ? "authorization"
    : existingAuthCookie
    ? "cookie"
    : undefined;
  ...
  if (msg.method !== "OPTIONS") {
    authResult = await runtimeConfig.authoriser.verifyJwtHeader(
      authHeader,
      authCookie,
      msg.url.path,
    );
  ...
  if (typeof authResult === "string") {
    ...
    const refreshTime = (msg.user.exp || 0) -
      (sessionTimeoutMins || 30) * 60 * 1000 / 2;
    const nowTime = new Date().getTime();
    if (nowTime > refreshTime) {
      const refreshUser = msg.user as AuthUser;
      const payload = buildJwtPayload(
        refreshUser,
        refreshUser as unknown as Record<string, unknown>,
        serviceConfig.jwtUserProps,
      );

...
function agentName(owner: Record<string, unknown>): string | undefined {
  const xAgent = isPlainObject(owner["x-agent"]) ? owner["x-agent"] : {};
  return typeof xAgent.name === "string" && xAgent.name.trim()
    ? xAgent.name.trim()
    : undefined;
}

function descriptionFromEntity(candidate: EntityCandidate): string | undefined {
  const xAgent = isPlainObject(candidate.schema["x-agent"])
    ? candidate.schema["x-agent"]
    : {};
  return typeof xAgent.description === "string" && xAgent.description.trim()
    ? xAgent.description
    : typeof candidate.schema.description === "string" &&
        candidate.schema.description.trim()
...
function validateAgentName(
  ...
  const xAgent = isPlainObject(owner["x-agent"]) ? owner["x-agent"] : {};
  if (typeof xAgent.name !== "string" || !xAgent.name.trim()) {
    issues.push(
      issue(
        "error",
        "agent_name_missing",
        "Exposed metadata must declare `x-agent.name`",
        sourceType,
        sourcePath,
        id,
      ),
...
function getExposeState(
  ...
  if (xExpose === undefined) {
    return { present: false, exposed: false, issues: [] };
  }
...
function propertyNames(schema: Record<string, unknown>): Set<string> {
  ...
  return new Set(
    isPlainObject(properties) ? Object.keys(properties) : [],
  );
}
...
function validateFieldList(
  ...
  if (
    ...
    issues.push(
      issue(
        "error",
        "field_list_malformed",
        `\`${fieldName}\` must be an array of field names`,
        "entity",
        sourcePath,
        id,
      ),
    ...
    if (!properties.has(field)) {
      issues.push(
        issue(
          ...
          `Field \`${field}\` in \`${fieldName}\` is not declared in schema properties`,
          "entity",
          sourcePath,
          id,
        ),
...
function validateSingleField(
  ...
  if (typeof value !== "string") {
    issues.push(
      issue(
        "error",
        "field_name_malformed",
        `\`${fieldName}\` must be a field name`,
        "entity",
        sourcePath,
        id,
      ),
  ...
  if (!properties.has(value)) {
    issues.push(
      issue(
        "error",
        "metadata_field_not_found",
        `Field \`${value}\` in \`${fieldName}\` is not declared in schema properties`,
        "entity",
        sourcePath,
        id,
      ),
...
function validateEntityCandidate(
  ...
  if (!expose.present || (!expose.exposed && expose.issues.length === 0)) {
    return false;
  }
  ...
  if (schema.type !== "object" || !isPlainObject(schema.properties)) {
    issues.push(
      issue(
        "error",
        "entity_schema_not_object",
        "Exposed entity schema must be an object schema with properties",
        "entity",
        sourcePath,
        id,
      ),
  ...
  validateAgentName(schema, "entity", sourcePath, id || undefined, issues);
  ...
  if (typeof schema.title !== "string" || !schema.title.trim()) {
    issues.push(
      issue(
        "warning",
        "entity_title_missing",
        "Entity schema should declare `title`",
        "entity",
        sourcePath,
        id,
      ),
  ...
  if (typeof xAgent.entityName !== "string" || !xAgent.entityName.trim()) {
    issues.push(
      issue(
        "warning",
        "entity_name_missing",
        "Entity metadata should declare `x-agent.entityName`",
        "entity",
        sourcePath,
        id,
      ),
  ...
  if (
    !Array.isArray(xAgent.summaryFields) || xAgent.summaryFields.length === 0
  ) {
    issues.push(
      issue(
        "warning",
        "entity_summary_fields_missing",
        "Entity metadata should declare `x-agent.summaryFields`",
        "entity",
        sourcePath,
        id,
      ),
  ...
  if (
    !Array.isArray(xAgent.searchableFields) ||
    xAgent.searchableFields.length === 0
  ) {
    issues.push(
      issue(
        "warning",
        "entity_searchable_fields_missing",
        "Entity metadata should declare `x-agent.searchableFields`",
        "entity",
        sourcePath,
        id,
      ),
  ...
  if (
    !Array.isArray(xAgent.filterableFields) ||
    xAgent.filterableFields.length === 0
  ) {
    issues.push(
      issue(
        "warning",
        "entity_filterable_fields_missing",
        "Entity metadata should declare `x-agent.filterableFields`",
        "entity",
        sourcePath,
        id,
      ),
  ...
  if (!isPlainObject(schema["x-render"])) {
    issues.push(
      issue(
        "warning",
        "entity_render_missing",
        "Entity metadata should declare `x-render` hints",
        "entity",
        sourcePath,
        id,
      ),
...
function validatePipelineCandidate(
  ...
  if (!Array.isArray(spec.pipeline)) {
    issues.push(
      issue(
        "error",
        "pipeline_missing_pipeline",
        "Exposed pipeline metadata must include a `pipeline` array",
        "pipeline",
        sourcePath,
        id,
      ),
  ...
  validateAgentName(spec, "pipeline", sourcePath, id || undefined, issues);
  ...
  if (xAgent.kind === undefined) {
    issues.push(
      issue(
        "error",
        "pipeline_kind_missing",
        "Exposed pipeline must declare `x-agent.kind`",
        "pipeline",
        sourcePath,
        id,
      ),
    ...
    issues.push(
      issue(
        "error",
        "pipeline_kind_invalid",
        "`x-agent.kind` must be `query` or `action`",
        "pipeline",
        sourcePath,
        id,
      ),
  ...
  if (xPolicy.effect !== "read") {
    issues.push(
      issue(
        "error",
        "pipeline_effect_not_read",
        'This discovery phase only exposes pipelines with `x-policy.effect = "read"`',
        "pipeline",
        sourcePath,
        id,
      ),
  ...
  if (spec.inputSchema !== undefined && !isPlainObject(spec.inputSchema)) {
    issues.push(
      issue(
        "error",
        "pipeline_input_schema_malformed",
  ...
  if (spec.outputSchema !== undefined && !isPlainObject(spec.outputSchema)) {
    issues.push(
      issue(
        "error",
        "pipeline_output_schema_malformed",
  ...
  if (typeof xAgent.title !== "string" || !xAgent.title.trim()) {
    issues.push(
      issue(
        "warning",
        "pipeline_title_missing",
        "Pipeline metadata should declare `x-agent.title`",
        "pipeline",
        sourcePath,
        id,
      ),
  ...
  if (typeof xAgent.description !== "string" || !xAgent.description.trim()) {
    issues.push(
      issue(
        "warning",
        "pipeline_description_missing",
        "Pipeline metadata should declare `x-agent.description`",
        "pipeline",
        sourcePath,
        id,
      ),
  ...
  if (!isPlainObject(spec.outputSchema)) {
    issues.push(
      issue(
        ...
        "Pipeline metadata should declare `outputSchema`",
        "pipeline",
        sourcePath,
        id,
      ),
  ...
  if (
    !Array.isArray(xAgent.suggestedUtterances) ||
    xAgent.suggestedUtterances.length === 0
  ) {
    issues.push(
      issue(
        "warning",
        "pipeline_suggested_utterances_missing",
        "Pipeline metadata should declare `x-agent.suggestedUtterances`",
        "pipeline",
        sourcePath,
        id,
      ),
  ...
  if (typeof xAgent.resultShape !== "string" || !xAgent.resultShape.trim()) {
    issues.push(
      issue(
        "warning",
        "pipeline_result_shape_missing",
        "Pipeline metadata should declare `x-agent.resultShape`",
        "pipeline",
        sourcePath,
        id,
      ),
...
function manifestModuleEndsWith(
  manifest: IServiceManifest,
  suffix: string,
): boolean {
  return typeof manifest.moduleUrl === "string" &&
    manifest.moduleUrl.replace(/\\/g, "/").endsWith(suffix);
}

async function getManifest(
  serviceConfig: IServiceConfig,
  tenantName: string,
): Promise<IServiceManifest | null> {
  const manifest = await runtimeConfig.modules.getServiceManifest(
    serviceConfig.source,
    tenantName,
  );
  if (typeof manifest === "string") {
    runtimeConfig.logger.error(
      `Failed to load manifest for ${serviceConfig.source}: ${manifest}`,
    );
    return null;
  }
  return manifest;
...
function pathInfos(value: unknown): PathInfo[] {
  return isPlainObject(value) &&
      Array.isArray((value as unknown as DirDescriptor).paths)
...
function entitySummary(candidate: EntityCandidate) {
  ...
  return {
    ...
    description: descriptionFromEntity(candidate),
...
function pipelineSummary(candidate: PipelineCandidate) {
  const xAgent = isPlainObject(candidate.spec["x-agent"])
    ? candidate.spec["x-agent"]
    : {};
  const xPolicy = isPlainObject(candidate.spec["x-policy"])
    ? candidate.spec["x-policy"]
    : {};
  return {
    name: candidate.id,
    ref: candidate.ref,
...
function listItem(candidate: EntityCandidate | PipelineCandidate) {
  ...
  return {
    name: candidate.id,
    description:
      typeof xAgent.description === "string" && xAgent.description.trim()
        ? xAgent.description
        : undefined,
  };
}

function listDocument(surface: Surface, scan: ScanResult) {
  return {
    surface,
    entities: scan.entities.map(listItem),
    queries: scan.pipelines
      .filter((candidate) => {
        const xAgent = isPlainObject(candidate.spec["x-agent"])
          ? candidate.spec["x-agent"]
          : {};
        return xAgent.kind === "query";
      })
      .map(listItem),
...
function logExcludedInvalid(
  context: SimpleServiceContext,
  candidate: EntityCandidate | PipelineCandidate,
) {
  const errors = candidate.issues.filter((i) => i.severity === "error");
  if (!errors.length) return;
  for (const err of errors) {
    context.logger.warn(
      `Agent surface excluded ${err.sourceType} ${candidate.sourcePath}: ${err.code} ${err.message}`,
    );
  }
}
...
async function collectDataEntities(
  ...
  const directory = await readJson(
    context,
    msg,
    `${serviceConfig.basePath}/?$list=details,all`,
  );
  ...
  for (const [name] of pathInfos(directory)) {
    ...
    const id = agentName(schema) ?? "";
    const candidate: EntityCandidate = {
      id,
      ref: typeof schema.$id === "string"
        ? schema.$id
        : `restspace://entity${serviceConfig.basePath}/${dataset}`,
...
function collectDatasetEntity(
  ...
  const schema =
    (serviceConfig as IServiceConfig & { schema?: unknown }).schema;
  const datasetName =
    (serviceConfig as IServiceConfig & { datasetName?: unknown }).datasetName;
  if (
    !isPlainObject(schema) || typeof datasetName !== "string" ||
    !datasetName.trim()
  ) {
    return [];
  }
  ...
  const candidate: EntityCandidate = {
    ...
    ref: typeof schema.$id === "string"
      ? schema.$id
      : `restspace://entity${serviceConfig.basePath}`,
    serviceBasePath: serviceConfig.basePath,
    dataset: datasetName,
...
function collectConfiguredPipeline(
  ...
  const spec: Record<string, unknown> = {
    ...
    inputSchema: configWithPipeline.inputSchema ??
      configWithPipeline.manualMimeTypes?.requestSchema,
    outputSchema: configWithPipeline.outputSchema ??
      configWithPipeline.manualMimeTypes?.responseSchema,
  ...
  const candidate: PipelineCandidate = {
    ...
    spec,
    pipeline: Array.isArray(configWithPipeline.pipeline)
      ? configWithPipeline.pipeline as PipelineSpec
      : [],
    inputSchema: isPlainObject(spec.inputSchema) ? spec.inputSchema : undefined,
    outputSchema: isPlainObject(spec.outputSchema)
      ? spec.outputSchema
      : undefined,
...
async function collectStoredPipelines(
  ...
  for (const [name] of pathInfos(directory)) {
    ...
    const pipeline = Array.isArray(rawSpec.pipeline)
      ? rawSpec.pipeline as PipelineSpec
      : [];
    const id = agentName(rawSpec) ?? "";
    const candidate: PipelineCandidate = {
      id,
      ref: typeof rawSpec.$id === "string"
        ? rawSpec.$id
        : `restspace://pipeline${specUrl}`,
      sourceKind: "store",
      serviceBasePath: serviceConfig.basePath,
      itemPath,
      ...
      spec: rawSpec,
      pipeline,
      inputSchema: isPlainObject(rawSpec.inputSchema)
        ? rawSpec.inputSchema
        : undefined,
      outputSchema: isPlainObject(rawSpec.outputSchema)
        ? rawSpec.outputSchema
        : undefined,
      sourcePath: specUrl,
      issues: [],
    };
...
function applyUniqueNameIssues(
  candidates: (EntityCandidate | PipelineCandidate)[],
  sourceType: SourceType,
) {
  const byName = new Map<string, (EntityCandidate | PipelineCandidate)[]>();
  for (const candidate of candidates) {
    if (!candidate.id) continue;
    const named = byName.get(candidate.id) ?? [];
    named.push(candidate);
    byName.set(candidate.id, named);
  }
  ...
  for (const [name, named] of byName) {
    ...
    for (const candidate of named) {
      candidate.issues.push(issue(
        "error",
        "agent_name_not_unique",
        `Exposed ${sourceType} name \`${name}\` must be unique`,
        sourceType,
...
async function scanSurface(
  ...
  for (const serviceConfig of Object.values(tenant.servicesConfig!.services)) {
    ...
    if (
      includeEntities &&
      ((manifest.apis || []).includes("data.base") ||
        manifestModuleEndsWith(manifest, "services/data.ts"))
    ) {
      candidates = await collectDataEntities(
        serviceConfig,
        context,
        msg,
        surface,
      ...
      includeEntities &&
      ((manifest.apis || []).includes("data.set") ||
        manifestModuleEndsWith(manifest, "services/dataset.ts"))
    ) {
      candidates = collectDatasetEntity(serviceConfig, surface);
      entities.push(...candidates as EntityCandidate[]);
    } else if (
      includePipelines &&
      ((manifest.apis || []).includes("pipeline") &&
        manifestModuleEndsWith(manifest, "services/pipeline.ts"))
    ) {
      candidates = collectConfiguredPipeline(serviceConfig, surface);
      pipelines.push(...candidates as PipelineCandidate[]);
    } else if (
      includePipelines &&
      manifestModuleEndsWith(manifest, "services/pipeline-store.ts")
    ) {
      candidates = await collectStoredPipelines(
        serviceConfig,
        context,
        msg,
        surface,
  ...
  applyUniqueNameIssues(entities, "entity");
  applyUniqueNameIssues(pipelines, "pipeline");

  if (logInvalid) {
    [...entities, ...pipelines].filter(hasErrors).forEach((candidate) =>
      logExcludedInvalid(context, candidate)
    );
  }
...
async function scanValidation(
  ...
  for (const serviceConfig of Object.values(tenant.servicesConfig!.services)) {
    ...
    if (
      ...
      rawCandidates = await collectDataEntities(
        serviceConfig,
        context,
        msg,
        surface,
      ...
      rawCandidates = await collectStoredPipelines(
        serviceConfig,
        context,
        msg,
        surface,
  ...
  const entityIssues = entities.flatMap((candidate) => candidate.issues);
  const pipelineIssues = pipelines.flatMap((candidate) => candidate.issues);
  const includedEntities = entities.filter((candidate) =>
    !hasErrors(candidate)
  );
  const includedPipelines = pipelines.filter((candidate) =>
    !hasErrors(candidate)
  );
  ...
  return {
    ...
    summary: {
      entities: {
        ...
        excluded: entityIssues.some((i) => i.severity === "error")
          ? new Set(
            entityIssues.filter((i) => i.severity === "error").map((i) =>
              i.id || i.sourcePath
            ),
          ).size
          : 0,
      ...
      pipelines: {
        included: includedPipelines.length,
        excluded: pipelineIssues.some((i) => i.severity === "error")
          ? new Set(
            pipelineIssues.filter((i) => i.severity === "error").map((i) =>
              i.id || i.sourcePath
            ),
          ).size
          : 0,
    ...
    entities: {
      included: includedEntities.map(entitySummary),
      issues: entityIssues,
    },
    pipelines: {
      included: includedPipelines.map(pipelineSummary),
...
function rootDocument(msg: Message, surface: Surface, scan: ScanResult) {
  ...
  return {
    ...
    list: {
      href: `${base}/list?surface=${surface}`,
    },
    capabilities: {
...
service.getDirectoryPath(
  "entities",
  async (msg: Message, context: SimpleServiceContext) => {
    ...
    return msg.setDirectoryJson({
      ...
      paths: scan.entities.map((entity) =>
        [entity.id, 0, viewItemSpec()] as PathInfo
      ),
      spec: viewStoreSpec(),
    } as DirDescriptor);
  },
...
service.getDirectoryPath(
  ...
  async (msg: Message, context: SimpleServiceContext) => {
    ...
    return msg.setDirectoryJson({
      path: msg.url.servicePath,
      paths: scan.pipelines.map((pipeline) =>
        [pipeline.id, 0, viewItemSpec()] as PathInfo
      ),
      spec: viewStoreSpec(),
    } as DirDescriptor);
  },
...
async function route(msg: Message, context: SimpleServiceContext) {
  ...
  if (path.length === 2 && path[0] === "pipelines") {
    const scan = await scanSurface(context, msg, surface, true, false, true);
    const pipeline = scan.pipelines.find((candidate) =>
      candidate.id === path[1]
    );
  ...
  if (path.length === 1 && path[0] === "list") {
    const scan = await scanSurface(context, msg, surface, true);
    return msg.setDataJson(listDocument(surface, scan));
  }

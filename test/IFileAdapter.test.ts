...
const testFileSpace = (adapter: IFileAdapter) => {
  ...
  Deno.test("saves file", async () => {
    await adapter.write(
      "abc-déf_ghi/jkl mno.html",
      new MessageBody(
        encoder.encode("<span>This is a file</span>").buffer as ArrayBuffer,
        "text/html",
      ),
    );
  });
  Deno.test("reads file", async () => {
    ...
    if (res.dateModified !== undefined) {
      ...
      assert(
        (now.valueOf() - res.dateModified.valueOf()) < 1000,
  ...
  Deno.test("writes two to directory", async () => {
    await adapter.write(
      "dir/item1.txt",
      new MessageBody(
        encoder.encode("An item").buffer as ArrayBuffer,
        "text/plain",
      ),
    );
    await adapter.write(
      "dir/item2.json",
      new MessageBody(
        encoder.encode('{ "abc": 2 }').buffer as ArrayBuffer,
        "application/json",
      ),
    );
  });
  Deno.test("writes subdirectory item", async () => {
    await adapter.write(
      "dir/subdir/item3.txt",
      new MessageBody(
        encoder.encode("Another item").buffer as ArrayBuffer,
        "text/plain",
      ),
    );
  });
...
testFileSpace(
  new LocalFileAdapter(makeAdapterContext("test"), {
    rootPath: "C:\\Dev\\test",
...
Deno.test("LocalFileAdapter serialises concurrent writes for same path", async () => {
  ...
  const body1 = new MessageBody(
    encoder.encode("1111111111").buffer as ArrayBuffer,
    "text/plain",
  );
  const body2 = new MessageBody(
    encoder.encode("2222222222").buffer as ArrayBuffer,
    "text/plain",
  );
...
Deno.test("LocalFileAdapter write returns 423 when path is locked by another write", async () => {
  ...
  const fastBody = new MessageBody(
    encoder.encode("fast").buffer as ArrayBuffer,
    "text/plain",
  );
...
Deno.test("LocalFileAdapter stores tenant paths invisibly below rootPath", async () => {
  const rootPath = await Deno.makeTempDir();
  try {
    const tenantA = new LocalFileAdapter(makeAdapterContext("tenant-a"), {
      rootPath,
      basePath: "shared",
    });
    const tenantB = new LocalFileAdapter(makeAdapterContext("tenant-b"), {
      rootPath,
      basePath: "shared",
    });

    await tenantA.write("item.txt", MessageBody.fromString("a"));
    await tenantB.write("item.txt", MessageBody.fromString("b"));

    assertEquals(await (await tenantA.read("item.txt")).asString(), "a");
    assertEquals(await (await tenantB.read("item.txt")).asString(), "b");
    assertEquals(
      await Deno.readTextFile(
        stdPath.join(rootPath, "tenant-a", "shared", "item.txt"),
      ),
      "a",
    );
    assertEquals(
      await Deno.readTextFile(
        stdPath.join(rootPath, "tenant-b", "shared", "item.txt"),
      ),
      "b",
    );
  } finally {
...
const getAWS4ProxyAdapter = <T extends IAdapter>(
  _url: string,
  config: unknown,
) =>
  Promise.resolve(
    getAdapterFromConfig("test", config, AWS4ProxyAdapter) as unknown as T,
  );

const HAS_AWS_CREDS = !!(Deno.env.get("AWS_ACCESS_KEY_ID") &&
  Deno.env.get("AWS_SECRET_ACCESS_KEY"));
if (HAS_AWS_CREDS) {
  testFileSpace(
    new S3FileAdapter(makeAdapterContext("test", getAWS4ProxyAdapter), {
      rootPath: "",

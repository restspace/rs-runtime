...
async function logIn(email: string, password = "hello") {
...
Deno.test("writes and reads file", async () => {
  ...
  const listDetails = msgOut.data
    ? await msgOut.data.asJson() as DirDescriptor
    : null;
  assertStrictEquals(listDetails?.paths[0][0], "abc.json");
  assert(
    typeof listDetails?.paths[0][1] === "number",
    "dateModified should be included with $list=details",
  );
  ...
  assertStrictEquals(
    msgOut.data?.mimeType,
    "inode/directory+json",
    "wrong mime type directory",
  );
...
Deno.test("file dirs", async () => {
  ...
  assert(
    list && list.length === 4,
    "recursive list should have 4 dir descriptors",
  );
  for (let i = 0; i < 4; i++) {
    assertEquals((list[i] as DirDescriptor).path, expectedRecursive[i].path);
    assertEquals(
      (list[i] as DirDescriptor).paths.length,
      expectedRecursive[i].pathNames.length,
    );
    for (let j = 0; j < expectedRecursive[i].pathNames.length; j++) {
      assertStrictEquals(
        (list[i] as DirDescriptor).paths[j][0],
        expectedRecursive[i].pathNames[j],
      );
      assert(
        typeof (list[i] as DirDescriptor).paths[j][1] === "number",
        `dateModified should be included for ${
          expectedRecursive[i].pathNames[j]
        }`,
      );
    }
  ...
  assertEquals(items, {
    "dira/dirb/xxx.json": { thing: "xxx" },
    "dirc/yyy.json": { thing: "yyy" },
  });
...
Deno.test("writes and reads file parentIfMissing", async () => {
  ...
  assertStrictEquals(
    str,
    '{ "thing": "def" }',
    "reads wrong body (parent if missing)",
  );
...
Deno.test("writes and reads data", async () => {
  ...
  assertStrictEquals(
    msgOut.data?.mimeType,
    'application/json; schema="http://basicChord.restspace.local:3100/data/set1/.schema.json"',
  );
  ...
  const listFull = msgOut.data
    ? await msgOut.data.asJson() as Record<string, unknown>
    : null;
...
Deno.test("writes and reads dataset", async () => {
  ...
  assertStrictEquals(
    msgOut.status,
    400,
    "Should give bad request for attempt to write schema",
  );

  await writeJson(
    "/data/ds/abc.json",
    { name: "fred", email: "fred@abc.com" },
  ...
  assertStrictEquals(
    msgOut.data?.mimeType,
    'application/json; schema="http://basicChord.restspace.local:3100/data/ds/.schema.json"',
  );
  assertStrictEquals(
    str,
    '{"name":"fred","email":"fred@abc.com"}',
    `reads wrong body: ${str}`,
  );
...
Deno.test("writes and reads users", async () => {
  ...
  assertStrictEquals(
    msgOut.data?.mimeType,
    'application/json; schema="http://basicChord.restspace.local:3100/user/.schema.json"',
  );
  assertStrictEquals(
    res.email,
    "jim_ej@hotmail.com",
    `reads wrong email: ${res.email}`,
  );
...
Deno.test("logs in", async () => {
  ...
  await writeJson(
    "/user/jamesej@outlook.com.json",
    user,
    "failed to write user",
  );

  const token = await logIn("jamesej@outlook.com.json", "hello123");
  ...
  assertStrictEquals(
    userOut.email,
    "jamesej@outlook.com",
    `reads wrong email: ${userOut.email}`,
  );
  assertStrictEquals(
    userOut.password,
    AuthUser.passwordMask,
    "password not mask after write mask",
  );
...
Deno.test("authenticated access", async () => {
  ...
  await writeJson(
    "/user-bypass/jamesej2@outlook.com.json",
    editor,
    "failed to write editor",
  );
  await writeJson(
    "/user-bypass/jamesej3@outlook.com.json",
    user,
    "failed to write user",
  );
  ...
  let msg = testMessage("/data/ds-auth/abc", "PUT", tokenUser).setDataJson({
    name: "bill",
    email: "bill@abc.dom",
  });
  let msgOut = await handleIncomingRequest(msg);
  assertStrictEquals(
    msgOut.status,
    401,
    "unauthorised user should fail to write",
  );

  msg = testMessage("/data/ds-auth/abc", "PUT").setDataJson({
    name: "bill",
    email: "bill@abc.dom",
  });
  msgOut = await handleIncomingRequest(msg);
  ...
  msg = testMessage("/data/ds-auth/abc", "PUT", tokenEditor).setDataJson({
    name: "bill",
    email: "bill@abc.dom",
  });
  msgOut = await handleIncomingRequest(msg);
  ...
  assertStrictEquals(
    msgOut.status,
    401,
    "unauthorised user should fail to delete",
  );
...
Deno.test("temporary access", async () => {
  ...
  await writeJson(
    "/user-bypass/jamesej4@outlook.com.json",
    editor,
    "failed to write editor",
  );
  ...
  await writeJson(
    "/data/ds-auth/ta 1.json",
    { name: "ta1", email: "fred@abc.com" },
    "failed to write data",
    tokenEditor,
  );
  await writeJson(
    "/data/ds-auth/ta2.json",
    { name: "ta2", email: "fred@abc.com" },
    "failed to write data",
    tokenEditor,
  );
  ...
  assertEquals(
    msgOut.status,
    403,
    "Should not authorize read outside base path",
  );
  ...
  assertEquals(
    msgOut.status,
    401,
    "Should not authorize read with invalid temp token",
  );
  ...
  assertEquals(
    msgOut.status,
    401,
    "Should not authorize read with expired temp token",
  );
  ...
  msg = testMessage(
    `/tempacc/${tempToken}/?path=data/ds-auth/ta 1`,
    "POST",
    tokenEditor,
  );
  msgOut = await handleIncomingRequest(msg);
  ...
  assertEquals(
    msgOut.status,
    403,
    "Should not authorize token deletion for anon user",
  );
  ...
  assert(
    msgOut.ok,
    "Should authorize read with valid temp token after failed deletion",
  );
  data = msgOut.data ? await msgOut.data.asJson() : null;
  assertEquals(data?.name, "ta1");
  ...
  assertEquals(
    msgOut.status,
    401,
    "Should not authorize read with cancelled temp token",
  );

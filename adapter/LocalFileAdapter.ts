...
import {
  ...
  toBlockChunks,
  writeFileStream,
} from "rs-core/streams/streams.ts";
...
class LocalFileAdapterBase implements IFileAdapter {
  ...
  constructor(
    ...
    public props: LocalFileAdapterProps,
  ) {
  ...
  getPathParts(
    ...
    if (ensureDirExists) {
      ensureDir(path.join(this.rootPath, ...pathParts.slice(0, -1)));
    }
    ...
    if (!forDir) {
      ...
      if (
        extensions.length &&
        (dotParts.length === 1 || !extensions.includes(pathExt))
      ) {
        ext = extensions[0];
  ...
  async read(
    ...
    try {
      ...
      if (err instanceof Deno.errors.NotFound) {
        return MessageBody.fromError(404);
      }
  ...
  async readDirectory(readPath: string, getUpdateTime = false) {
    ...
    try {
      ...
      return (err instanceof Deno.errors.NotFound)
        ? MessageBody.fromError(404)
        : MessageBody.fromError(500);
    }
    if (!stat.isDirectory) return MessageBody.fromError(400);

    const blockIter = toBlockChunks(
      this.dirIter(filePath, getUpdateTime || false),
    );

    return new MessageBody(ReadableStream.from(blockIter), "text/plain")
      .setIsDirectory();
  }
  ...
  async deleteDirectory(
    ...
    for await (const entry of Deno.readDir(filePath)) {
      if (
        entry.isDirectory ||
        !(deleteableFileSuffix && entry.name.endsWith(deleteableFileSuffix))
      ) {
        return 400;
      }
    }
  ...
  async move(fromPath: string, toPath: string, extensions?: string[]) {
    ...
    try {
      try {
        firstRelease = await acquireWriteLock(
          firstPath,
          this.writeLockTimeoutMs,
        );
        if (secondPath !== firstPath) {
          secondRelease = await acquireWriteLock(
            secondPath,
            this.writeLockTimeoutMs,
          );
        }

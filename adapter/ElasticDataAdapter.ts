...
import { prefixStorageName, unprefixStorageName } from "./tenantStorage.ts";
  ...
  switch (schema.type as string) {
    case "string": {
      submapping = schema.search === "textual"
        ? { type: "text" }
        : ["date", "date-time"].includes(schema.format || "zzz")
        ? { type: "date" }
        : { type: "keyword" };
      [
        "fields",
...
export default class ElasticDataAdapter
  ...
  schemaIndexName() {
    return this.physicalIndexName(".schemas");
  }
  ...
  async readKey(
    ...
    dataset = this.physicalIndexName(dataset);
    ...
    if (dataset === "") {
      ...
      if (!msgOut.ok) {
        ...
        const listing = Object.keys(data)
          .map((k) => this.logicalIndexName(k))
          .filter((k): k is string => k !== null && k !== ".schemas")
          .map((k) => [k + "/"] as PathInfo);
        return listing;
      }
    } else {
      ...
      if (msgOut.status === 404) {
        ...
        const data = await msgOut.data?.asJson();
        const listing = (data.hits.hits as any[]).map((h: any) =>
          (h.fields._timestamp
            ? [h._id, h.fields._timestamp]
            : [h._id]) as PathInfo
        );
  ...
  async ensureSchemasIndex() {
    if (!this.schemasIndexChecked) {
      const schemasIndex = this.schemaIndexName();
      const msg = new Message(
        `/${schemasIndex}`,
        this.context.tenant,
        "GET",
      ...
      if (!msgCheck.ok) {
        const createMappingMsg = new Message(
          `/${schemasIndex}`,
          this.context.tenant,
          "PUT",
          null,
        ...
        if (!msgCreated.ok) {
          this.context.logger.error(
            `Failed to create .schemas index on ${this.props.host}, request status ${msgCreated.status}`,
          );
        }
      }
  ...
  async writeSchema(
    ...
    const logicalDataset = this.normaliseIndexName(dataset);
    dataset = this.physicalIndexName(dataset);
    ...
    this.context.logger.info(
      `Writing mapping for ${dataset}: ${JSON.stringify(params.mappings)}`,
      ...contextLoggerArgs(this.context),
    );
    const msg = new Message(`/${dataset}`, this.context.tenant, "GET", null);
    const msgCheck = await this.requestElastic(msg);
    const setMappingMsg = new Message(
      `/${dataset}/_mapping`,
      this.context.tenant,
      "PUT",
      null,
    ...
    const storeRes = await this.writeKey(
      ".schemas",
      logicalDataset,
      MessageBody.fromObject({ schema: JSON.stringify(schema) }),
    );
    if (storeRes >= 300) {
      return storeRes;
    }
